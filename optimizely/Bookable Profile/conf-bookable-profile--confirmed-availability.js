...
console.log(todaysDate, isThisWeek);
...
function setConfirmedAvailability(startDateToConfirm, endDateToConfirm, confirmedStatus) {
	...
	setTimeout(function() {
		$.ajax({
			...
			// localStorage.setItem('has-confirmed-availability', confirmedStatus);
			$('#dv-checkable-buttons', 'body').remove();
			$('#dv-checkable', 'body').addClass('complete');
			if(confirmedStatus) {
				$('#dv-confirm-title', 'body').text('Great; thanks for the heads up!');
				$('#dv-checkable-microcopy', 'body').html('We&rsquo;ll elevate your profile in search. Manage your <a href="/calendar" target="_blank">Calendar</a>').addClass('show');
			} else {
